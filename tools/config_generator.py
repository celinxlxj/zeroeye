...
ConfigScalar = Optional[Union[str, int, float, bool]]
ConfigValue = Union[ConfigScalar, List[str], Dict[str, "ConfigValue"]]
...
ENV_OVERRIDES: Dict[str, ConfigDict] = {
    "development": {
...
def generate_config(env: str, overrides: Optional[ConfigDict] = None) -> ConfigDict:
...
def mask_sensitive(config: ConfigDict, prefix: str = "") -> ConfigDict:
    masked: ConfigDict = {}
...
def to_yaml(config: ConfigDict) -> str:
    if not HAS_YAML:
        return "ERROR: PyYAML is not installed"
    return yaml.dump(config, default_flow_style=False, sort_keys=False)


def to_json(config: ConfigDict, pretty: bool = True) -> str:
    if pretty:
        return json.dumps(config, indent=2, default=str)
    return json.dumps(config, default=str)


def to_toml(config: ConfigDict) -> str:
    if not HAS_TOML:
        return "ERROR: toml is not installed"

    def flatten(config: ConfigDict, prefix: str = "") -> ConfigDict:
        result: ConfigDict = {}
        for key, value in config.items():
            full_key = f"{prefix}.{key}" if prefix else key
...
def to_k8s_configmap(config: ConfigDict, name: str = "app-config") -> str:
    data_lines = []
...
def flatten_for_k8s(config: ConfigDict, prefix: str = "") -> List[Tuple[str, ConfigValue]]:
